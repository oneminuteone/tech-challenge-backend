...
describe('plugin', () => describe('movie', () => {
  ...
  describe('POST /movies', () => {
    ...
    it('returns HTTP 409 when exact movie record already exists', async ({ context }: Flags) => {
      if(!isContext(context)) throw TypeError()
      const payload = {'name': 'any-name', 'synopsis': 'some synopsis', 'releasedAt': new Date('1985-05-02'), 'runtime': 155, 'genre_id': 1}
      const opts: Hapi.ServerInjectOptions = { method, url, payload }
  ...
  describe('PUT /movies/:id', () => {
    ...
    const payloadObj = {'name': 'any-name', 'synopsis': 'some synopsis', 'releasedAt': new Date('1985-05-02'), 'runtime': 155}
    const [method, url, payload] = ['PUT', `/movies/${paramId}`, payloadObj]
